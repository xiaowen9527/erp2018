<template>
    <div class="navMenu">
        ...
        <label v-for="(navMenu,i) in navMenus" :key="i">
            <!--只有一级菜单-->
            <el-menu-item v-if="navMenu.childs.length==0" :key="navMenu.entity.bhGs" :index="String(navMenu.entity.id)">
                <!--标题-->
                <span slot="title">|--{{navMenu.entity.name}}</span>
            </el-menu-item>
            <!--有多级菜单-->
            <el-submenu v-if="navMenu.childs.length!=0" :key="navMenu.entity.bhGs" :index="String(navMenu.entity.id)">
                <template slot="title">
