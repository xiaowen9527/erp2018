					...
					<section class="firstForm">
						<ul class="clearfix">
							<li>
								...
								<el-select class="size" v-model="sizeName" :disabled="firstFormGui" placeholder="请选择尺码组">
									<el-option v-for="item in sizeNameList" :key="item.value" :label="item.label" :value="item.value"> </el-option>
								</el-select>								
								<!-- <input :disabled="!sizeGroup" type="text" readonly placeholder="请选择" @click="doSize" v-model="size.name"> -->
...
export default {
	...
	methods: {
		...
		getAllSize(){
			this.$http.post('/TPA/cSpdaB/geyByPsn')
				.then(res=>{
					if(res.data.code===0){
