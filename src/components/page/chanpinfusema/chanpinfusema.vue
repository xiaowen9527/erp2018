  ...
  >
    ...
    <div class="set_box">
      <div class="menu_box">
        ...
        >
          <el-option
            v-for="item in this.searchYearList"
            :key="item.name"
            :label="item.name"
            :value="item.name"
          >
            ...
            <ul class="clearfix">
              <li>
                ...
                <el-select
                  class="size"
                  v-model="sizeGroup"
                  :disabled="firstFormGui"
                  placeholder="请选择尺码组"
                >
                  <el-option
                    v-for="item in sizeGroupList"
                    :key="item.name"
                    :label="item.name"
                    :value="item.name"
                  > </el-option>
                ...
                >
                  <template slot-scope="scope">
                    <el-button
                      ...
                      :class="{btn:(scope.row.status == 1)}"
                      @click="effectiveSize(scope.$index, scope.row)"
                      type="text"
                    >有效</el-button>
                    <el-button
                      :disabled="(scope.row.status == 0)"
                      :class="{btn:(scope.row.status == 0)}"
                      @click="invalidSize(scope.$index, scope.row)"
                      type="text"
                    >无效</el-button>
                  </template>
                </el-table-column>
          ...
          <section class="firstForm">
            <ul class="clearfix">
              <li>
                ...
                <button
                  :disabled="secondFormGui"
                  :class="{btn:!secondFormGui}"
                  @click="doColor"
                >选择</button>
                <button
                  :disabled="!secondSave"
                  :class="{btn:secondSave}"
                  @click="doSecondSave"
                >保存</button>
                <button
                  class="btn"
                ...
                >
                  <template slot-scope="scope">
                    <el-button
                      :disabled="(scope.row.status == 1)"
                      :class="{btn:(scope.row.status == 1)}"
                      @click="effectiveColor(scope.$index, scope.row)"
                      type="text"
                    >
                      有效</el-button>
                    <el-button
                      :disabled="(scope.row.status == 0)"
                      :class="{btn:(scope.row.status == 0)}"
                      @click="invalidColor(scope.$index, scope.row)"
                      type="text"
                    >无效</el-button>
                  </template>
                </el-table-column>
    ...
    <el-dialog
      title="查询"
      :visible.sync="oldSearch"
    >
      <el-input
        v-model="searchXun"
        placeholder="请输入你要查找的款号"
      ></el-input>
    ...
    <el-dialog
      title="颜色"
      :visible.sync="oldColor"
    >
      <el-input
        v-model="searchColor"
        placeholder="请输入你要查找的颜色"
      ></el-input>
      <ul class="srcond_menu">
...
export default {
  ...
  methods: {
    ...
    //获取所有尺码组
    getAllSize() {
      this.$http.post('/TPA/cSpdaB/getByPsn')
        .then(res => {
          if (res.data.code === 0) {
    ...
    //获取sn尺码组下的所有尺码
    getSize(sn) {
      this.$http.post('/TPA/cSpdaB/getBy?sn=' + sn)
        .then(res => {
          if (res.data.code === 0) {
