            ...
            <div class="set_info">
                <div class="info_form">
                    <ul>
                        <li class="menuLi">
                            <label>上级</label>
                            <input v-model="form.pidSn" type="text" class="menuLiNum" readonly :disabled='menuBtn' placeholder="顶级">
                            <input v-model="form.pidName" type="text" readonly :disabled='menuBtn' placeholder="顶级">
...
export default {
  ...
  methods: {
    ...
    uploadFile(params){
        ...
        //是否覆盖
        if(this.fugai){
            this.$ajax.post('/TPA/aKsDa/importExcel?isCover=true',formData)
                .then(res=>{
                    console.log(res);
                if(res.status===200){
                    if(res.data.code===0){
            ...
            this.$ajax.post('/TPA/aLbCh/importExcel?isCover=false',formData)
                .then(res=>{
                if(res.status===200){
                    if(res.data.code===0){
                            succ(res.data.msg)
