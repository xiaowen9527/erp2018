...
export default {
    ...
    methods: {
        ...
        saveEdit() {
            ...
            let term = !this.dialog.amount||!this.dialog.nature||!this.dialog.productSn||!this.dialog.func||!this.dialog.part||!this.dialog.loss||!this.dialog.dosage||!this.dialog.dosageUnit||!this.dialog.dosageUnitSn||!this.dialog.explain           
           
            if(!term){
                this.$http.post('/TPA/cMatBill/update',qs.stringify(this.dialog))
                        .then(res=>{
                            console.log(res)
                            if(res.data.code===0){
        ...
        getDesignColor(psn){
            ...
            this.$http.post('/TPA/cMatBillA/getBy?psn='+psn)
                .then(res=>{
                    if(res.data.code===0){
                        this.DesignColor = res.data.data

                        for(let i=0;i<this.list.length;i++){
                            this.$http.post('/TPA/cMatBill/getBySn?sn='+this.list[i].sn)
                                .then(res=>{
                                    if(res.data.code===0){
