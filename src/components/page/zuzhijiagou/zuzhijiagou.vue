        ...
        <div class="set_box">
            ...
            <div class="set_info">
                <ul>
                    ...
                    <li class="company">
                        <label>归属公司</label>
                        <input type="text" readonly class="companyNum" v-model="form.gsSn" :disabled='gui' placeholder="顶级">
                        <input type="text" readonly class="companyName" v-model="form.gsName" :disabled='gui' placeholder="顶级">
...
export default {
  ...
  methods: {
    ...
    uploadFile(params){
        ...
        //是否覆盖
        if(this.fugai){
            this.$ajax.post('/TPA/aGsZzjg/importExcel?isCover=true',formData)
                .then(res=>{
                    if(res.status===200){
                        if(res.data.code===0){
                                succ(res.data.msg)
            ...
            this.$ajax.post('/TPA/aGsZzjg/importExcel?isCover=false',formData)
                .then(res=>{
                    if(res.status===200){
                        if(res.data.code===0){
                                succ(res.data.msg)
